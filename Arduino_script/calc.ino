void calc()
{
  ...
  {
    Ri = 1000 * ((float)(Voltagei - Voltage) / Current * 100);
    Voltagei = 0;
    if ((flag & 0x08) == 0)
      flag += 0x08;
  }
  ...
  //---------------------------------------------电池温度
  MaxBatProbTemp[4] = -50;
  MinBatProbTemp[4] = 120;
  for (i = 0; i <= 3; i++)
  {
    if (MaxBatProbTemp[4] < MaxBatProbTemp[i])
      MaxBatProbTemp[4] = MaxBatProbTemp[i];
    if (MinBatProbTemp[4] > MinBatProbTemp[i])
      MinBatProbTemp[4] = MinBatProbTemp[i];
  }
  //---------------------------------------------单体电压最值
  MaxVolt[4] = 10000;
  MinVolt[4] = 50000;
  ...
  for (i = 0; i <= 3; i++)
  {
    if (MaxVolt[4] < MaxVolt[i])
      MaxVolt[4] = MaxVolt[i];
    if (MinVolt[4] > MinVolt[i])
      MinVolt[4] = MinVolt[i];
  }
